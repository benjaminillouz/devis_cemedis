...
function setupEventListeners() {
    ...
    if (signBtn) {
        signBtn.addEventListener('click', handleSignature);
    }
    ...
    if (typeof createChat === 'function') {
        try {
            createChat({
                ...
                title: 'Bonjour',
                metadata: {
                    "patient_id": getUrlParameter('idPat') || getUrlParameter('idpat') || '',
                    "patient_nom": decodeURIComponent(getUrlParameter('patientnom') || ''),
                    "patient_prenom": decodeURIComponent(getUrlParameter('patientprenom') || ''),
                    "id_devis": getUrlParameter('idDevis') || getUrlParameter('id') || '',
                    "total": getUrlParameter('total') || '',
                    "mut": getUrlParameter('mut') || '',
                    "rac": getUrlParameter('rac') || '',
                    "ss": getUrlParameter('ss') || '',
                    "centre": decodeURIComponent(getUrlParameter('centre') || ''),
                    "praticien": decodeURIComponent(getUrlParameter('praticien') || '')
                },
                showWelcomeScreen: false,
                chatSessionKey: `${getUrlParameter('idPat') || getUrlParameter('idpat') || 'unknown'}-${getUrlParameter('idDevis') || getUrlParameter('id') || 'unknown'}`,
                defaultLanguage: 'fr',
                inputPlaceholder: 'saisir votre message',
                initialMessages: [
                    `Bonjour ${decodeURIComponent(getUrlParameter('patientnom') || '')} ${decodeURIComponent(getUrlParameter('patientprenom') || '')}`,
                    'Comment puis-je vous aider avec votre devis?'
                ],
                i18n: {
                    fr: {
                        title: 'Votre assistant devis ‍⚕️',
                        subtitle: "",
                        getStarted: 'New Conversation',
                        inputPlaceholder: 'Posez moi vos questions',
                    },
                },
            });
        } catch (error) {
...
function sendMessage() {
    ...
    setTimeout(() => {
        const responses = [
            'Je comprends votre question. Laissez-moi vous aider avec votre devis.',
            'C\'est une excellente question ! Voici les informations que je peux vous donner.',
            'Je vais vous expliquer cela en détail.',
            'Pourriez-vous préciser votre question ? Je serais ravi de vous aider.'
        ];
...
function sendMessage() {
    ...
    if (input.value.trim()) {
        ...
        setTimeout(() => {
            ...
            botMessage.innerHTML = '<p>Merci pour votre message ! Notre équipe vous répondra rapidement.</p>';
