...
// Initialisation du bouton de chat
function initializeChatButton() {
    ...
    if (chatButton) {
        chatButton.addEventListener('click', function(e) {
            ...
            try {
                // Méthode 1: Essayer d'ouvrir directement via l'instance n8n
                if (window.n8nChatInstance) {
                    console.log('Instance n8n trouvée:', window.n8nChatInstance);
                    
                    // Essayer différentes méthodes d'ouverture
                    if (typeof window.n8nChatInstance.open === 'function') {
                        window.n8nChatInstance.open();
                        return;
                    } else if (typeof window.n8nChatInstance.show === 'function') {
                        window.n8nChatInstance.show();
                        return;
                    } else if (typeof window.n8nChatInstance.toggle === 'function') {
                        window.n8nChatInstance.toggle();
                        return;
                    }
                }
                
                // Méthode 2: Essayer de trouver et cliquer sur le bouton n8n natif
                setTimeout(() => {
                    const n8nChatButton = document.querySelector('.chat-window-toggle, .chat-toggle, [data-chat-toggle], .chat-window-wrapper .chat-window-toggle');
                    if (n8nChatButton) {
                        console.log('Bouton n8n natif trouvé, clic simulé');
                        n8nChatButton.click();
                    } else {
                        console.log('Aucun bouton n8n natif trouvé');
                    }
                }, 100);
