...
async function handleFinancing(paymentMethod) {
    ...
    try {
        ...
        const paymentLink = response?.link || response?.url || response?.payment_url || response?.checkout_url;
        
        if (paymentLink) {
            window.location.href = paymentLink;
        } else {
            console.error('Structure de réponse inattendue:', response);
            throw new Error('Aucun lien de paiement reçu dans la réponse. Réponse: ' + JSON.stringify(response));
        }
        
    } catch (error) {
        console.error('Erreur lors du financement:', error);
        const errorMessage = error.message || 'Erreur lors de la création du lien de paiement. Veuillez réessayer.';
...
async function handleVeasyFinancing(amount) {
    ...
    const requiredParams = ['idPraticien', 'idCentre', 'patientId'];
    const missingParams = requiredParams.filter(param => {
        const key = param === 'patientId' ? 'idPat' : param;
...
async function handleStripeFinancing(amount, paymentMethod) {
    ...
    const payload = {
        ...
        "amount": amount,
        "payment_method_types": [mappedMethod], // Envoyer comme tableau pour éviter la transformation en hash
        "itemid": getUrlParameter('idPodio'),
        "beneficiary_reference": getUrlParameter('br')
    };
    
    // Validation des paramètres essentiels
    const requiredParams = ['idPraticien', 'idCentre', 'idPat'];
    const missingParams = requiredParams.filter(param => !getUrlParameter(param));
...
async function makeApiCall(url, payload) {
    try {
        ...
        console.log('Réponse API (body):', responseText);
        ...
        try {
            ...
            throw new Error(`Erreur de parsing JSON: ${e.message}. Réponse: ${responseText.substring(0, 100)}`);
        }
